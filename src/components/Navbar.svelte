...
<div class="flex">
    ...
    <div class="flex gap-5">
      <div
        aria-current={$page.url.pathname.startsWith('/login')
          ...
          : undefined}
      >
        <a href="/login" class="hover:text-purple-800 hover:duration-150"
          >Login</a
        >
      </div>
      <div
        aria-current={$page.url.pathname.startsWith('/register')
          ...
          : undefined}
      >
        <a href="/register" class="hover:text-purple-800 hover:duration-150"
          >Create Account</a
        >
      </div>
      <div>
  ...
  <nav class="md:hidden flex w-full justify-between items-center mx-5 py-8">
    ...
    <div>
      <button
        on:click={openBurgerMenu}
        class=" border px-3 p-1 text-2xl rounded-md"
      >
        {#if isBurgerMenu}
    ...
    {#if isBurgerMenu}
      <div
        class="absolute top-20 right-10 bg-white dark:bg-[#1B1D21] shadow-lg border border-gray-100 p-3 rounded-lg py-4 dark:border-[#606267] dark:shadow-md z-10 w-56"
      >
        <div class="flex flex-col gap-4 px-3">
          ...
          >
            ...
            <a
              href="/login"
              on:click={closeMenu}
              class="hover:text-purple-800 hover:duration-150 text-md">Login</a
            >
          </div>
          <div
            class="flex gap-4 items-center"
          ...
          >
            ...
            <a
              href="/register"
              on:click={closeMenu}
              class="hover:text-purple-800 hover:duration-150 text-md"
              >Create Account</a
            >
          </div>
          <div>
            ...
            >
              {#if darkMode}
                <i
                  class="fa-solid fa-sun hover:border hover:rounded-full hover:bg-slate-400"
                ></i>
              {:else}
                <i class="fa-solid fa-moon"></i>
              {/if}
