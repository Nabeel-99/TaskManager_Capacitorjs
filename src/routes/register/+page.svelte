...
const handleSignUp = async () => {
 ...
 <div class="flex flex-col gap-8 lg:gap-8  lg:flex-row min-h-[100vh] transition-all duration-500 ease-in-out    p-10 rounded-md items-center lg:justify-around px-14 bg-[#f5f5f5] dark:bg-[#2f3134] ">
     <div class="order-2 lg:order-none border p-8 flex flex-col gap-8 rounded-md shadow-md bg-[#edf0f0] md:w-auto w-screen mt-10  dark:bg-[#232529] dark:text-white dark:border-[#454648]">
         ...
         <form class="flex flex-col gap-3 border p-5 border-black rounded-md  bg-white  dark:text-black">
            {#if authError}
            ...
            <input bind:value={email} type="email" id="emailInput" class="border md:w-96 rounded-sm px-2 p-1" required/>
             ...
             <input bind:value={firstName}  type="text" id="firstname" class="border md:w-96 rounded-sm px-2 p-1" required/>
             <label for="lastname">Last Name:</label>
             <input bind:value={lastName} type="text" id="lastname" class="border md:w-96 rounded-sm px-2 p-1" required/>
