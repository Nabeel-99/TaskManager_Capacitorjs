 ...
 <div class="flex flex-col gap-8 lg:gap-8  lg:flex-row min-h-[100vh]  p-10 rounded-md items-center lg:justify-around px-14 bg-[#f5f5f5] dark:bg-[#2f3134] ">
     <div class="order-2 lg:order-none border border-black p-8 flex flex-col gap-8 rounded-md shadow-md bg-[#edf0f0] lg:w-1/2 w-screen mt-10  dark:bg-[#232529] dark:text-white dark:border-[#454648]">
         ...
         <form class="flex flex-col gap-3 border p-5 border-black rounded-md  bg-white  dark:white dark:bg-[#2A2D32] dark:border-white">
            ...
            <input bind:value={email} type="email" id="emailInput" class="border md:w-full rounded-sm px-2 p-1 dark:bg-[#2A2D32]" required/>
            
             <label for="firstname">First Name:</label>
             <input bind:value={firstName}  type="text" id="firstname" class="border md:w-full rounded-sm px-2 p-1 dark:bg-[#2A2D32]" required/>
             <label for="lastname">Last Name:</label>
             <input bind:value={lastName} type="text" id="lastname" class="border md:w-full rounded-sm px-2 p-1 dark:bg-[#2A2D32]" required/>
             <label for="password">Password:</label>
             <input bind:value={password} type="password" id="password" class="border md:w-full rounded-sm px-2 p-1 dark:bg-[#2A2D32]" required/>
             <div>
